...
// ErrorException
//
// Exception thrown if error occurs while running DragonflyRepoAgent API.
//
...
class FileSystem {
public:
    FileSystem(const std::string& path, const S3Credential& cred);
    ...
    TRITONSERVER_Error* CleanPath(const std::string& path, std::string* clean_path);
    TRITONSERVER_Error* LocalizePath(const std::string& path,const std::string& l);
